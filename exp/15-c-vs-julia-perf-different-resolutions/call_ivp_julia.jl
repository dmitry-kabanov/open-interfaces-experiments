...
function benchmark_this_version(func, N::Int, save_ic::Bool, save_solution::Bool)
    a = 0.0
    b = 2.0
    dx = (b - a) / N
    ...
    if save_ic
        ic_filename = @sprintf("_output/N=%04d/ic-julia.txt", N)
...
function measure(N::Int, N_TRIALS::Int)
    ...
    save_ic = false
    save_solution = false
    runtime = benchmark_this_version(w1, N, save_ic, save_solution)
