...
int
parse_n_trials(int argc, char *argv[]) {
    assert(argc == 3);
...
int
benchmark_one_run(
    const char *impl, const char *ic_filename, bool save_ic, const char *solution_filename, int N, bool save_solution, double *p_runtime)
{
    int retval = -1;
    double t0 = 0.0;
    double t_final = 10.0;
    ...
    if (save_ic) {
        ...
        for (int i = 0; i <= N; ++i) {
            fprintf(fp, "%.16f %.16f\n", grid->data[i], y0->data[i]);
        }
        fclose(fp);
        printf("Initial condition was written to file `%s`\n", ic_filename);
    }
...
main(int argc, char *argv[])
{
    ...
    const int N_TRIALS = parse_n_trials(argc, argv);
    int nbytes_written = -1;

    char ic_filename[BUFFER_SIZE];
    char ic_fmt[] = "_output/N=%04d/ic-c.txt";
    nbytes_written = snprintf(ic_filename, sizeof ic_filename, ic_fmt, N);
    if (nbytes_written < 0 || nbytes_written >= sizeof ic_filename) {
        fprintf(
            stderr,
            "[main] Cannot format `ic_filename`: "
    ...
    char solution_filename[BUFFER_SIZE];
    char fmt[] = "_output/N=%04d/solution-c.txt";
    nbytes_written = snprintf(solution_filename, sizeof solution_filename, fmt, N);
    ...
    char runtimes_filename[BUFFER_SIZE];
    ...
    printf("=== BEGIN warmup\n");
    benchmark_one_run(impl, ic_filename, save_ic, solution_filename, N, save_solution, &runtimes[0]);
    printf("=== END warmup\n");
    ...
    for (int i = 0; i < N_TRIALS; ++i) {
        ...
        int status = benchmark_one_run(impl, ic_filename, save_ic, solution_filename, N, save_solution, &runtimes[i]);
        assert(status == 0);
    }
