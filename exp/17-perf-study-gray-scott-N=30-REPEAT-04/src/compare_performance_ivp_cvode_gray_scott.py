...
RESULT_PERF_FILENAME = os.path.join("_assets", "ivp_cvode_gs_perf.pdf")
RESULT_PERF_NORM_FILENAME = os.path.join("_assets", "ivp_cvode_gs_perf_normalized.pdf")
...
class GrayScottProblem:
    ...
    def __init__(
        self, N=64, tfinal=10000, left=-2.5, right=2.5, bottom=-2.5, top=2.5, seed=None
    ):
        self._count = 0
        ...
        # Number of points around center that are in a perturbed state.
        p = 10
        s = N // 2
        U[s - p : s + p, s - p : s + p] = 0.50
        V[s - p : s + p, s - p : s + p] = 0.25

        if seed is not None:
            np.random.seed(seed)
        U[s - p : s + p, s - p : s + p] += 0.01 * np.random.rand(2 * p, 2 * p)
        V[s - p : s + p, s - p : s + p] += 0.01 * np.random.rand(2 * p, 2 * p)
    ...
    for impl in IMPL_LIST:
        for N in RESOLUTIONS:
            for __ in range(args.n_runs):
                elapsed_time = _run_once(impl, N=N, plot_solution=False)
...
def print_stats(tts_stats):
    ...
    print(
        "{:^24s} ".format("N")
        + col_sep.join(["{:^14d}".format(N) for N in RESOLUTIONS])
    )
    for impl in IMPL_LIST:
        print(
            "{:24s} ".format(impl)
...
def plot(tts_stats):
    ...
    plt.legend(loc="best")
    plt.tight_layout(pad=0.1)
    plt.savefig(RESULT_PERF_NORM_FILENAME)


def _run_once(args, N=256, tfinal=10000, plot_solution=True) -> float:
    ...
    if impl == "native_sundials_cvode":
        ...
        s.set_tolerances(rtol=RTOL, atol=ATOL)
    ...
    np.savetxt(RESULT_SOLN_DATA_FILENAME_TPL.format(impl), soln[-1])
