...
RESULT_FILENAME_SUNDIALS = OUTDIR * "/runtime_vs_resolution_julia_sundials.csv"
...
function measure_perf_once(N, inline)
    ...
    toc = time_ns()
    solution_last_1 = solution.u[end]
    runtime_1 = toc - tic

    tic = time_ns()
    solution = solve(odeProblem, CVODE_Adams(), reltol = 1e-6, abstol = 1e-12, saveat=times)
    toc = time_ns()
...
function main()
    ...
    table = ["method/resolution", "julia-DP5", "julia-Sundials-CVODE_Adams"]
    ...
    for N in RESOLUTIONS_LIST
        ...
        @printf "--- Resolution %d, using julia-DP5\n" N
        ...
        @printf "--- Resolution %d, using julia-Sundials-CVODE-Adams\n" N
        @printf "Runtime, sec: %.4f ± %.4f\n" runtime_mean_2 ci_2
        @printf "Solution second point from the left value: %.16f\n" solution_last_2[2]
    ...
    CSV.write(RESULT_FILENAME_SUNDIALS, df)
    println("Performance study results are written to the file ", RESULT_FILENAME_SUNDIALS)
end
