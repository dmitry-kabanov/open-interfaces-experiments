...
char *parse_output_filename(int argc, char *argv[]) {
  if (argc == 1 || argc == 2) {
    return "solution.txt";
  } else {
    return argv[2];
...
static int compute_initial_condition_(size_t N, OIFArrayF64 *u0,
                                      OIFArrayF64 *grid, double *dx,
                                      double *dt_max) {
  double a = 0.0;
  double b = 2.0;
  double *x = grid->data;
...
int rhs(double t, OIFArrayF64 *y, OIFArrayF64 *rhs_out, void *user_data) {
  (void)t;         /* Unused */
